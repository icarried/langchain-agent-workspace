from __future__ import annotations

import copy
import json
import re
from pathlib import Path
from typing import Any

from .schemas import ParsedImageToVideoRequest

REQUIRED_NODES = {
    "269": "LoadImage",
    "320:276": "RandomNoise",
    "320:277": "RandomNoise",
    "320:299": "PrimitiveInt",
    "320:300": "PrimitiveInt",
    "320:301": "PrimitiveInt",
    "320:312": "PrimitiveInt",
    "320:313": "CLIPTextEncode",
    "320:319": "PrimitiveStringMultiline",
    "320:328": "PrimitiveBoolean",
    "75": "SaveVideo",
}
VIDEO_ID_PATTERN = re.compile(r"^video_[A-Za-z0-9_-]+$")


class WorkflowTemplateError(RuntimeError):
    pass


class ImageToVideoWorkflowRenderer:
    def __init__(self, path: Path) -> None:
        self.path = path
        self.template = self._load(path)
        self._validate(self.template)

    @staticmethod
    def _load(path: Path) -> dict[str, Any]:
        try:
            value = json.loads(path.read_text(encoding="utf-8"))
        except FileNotFoundError as exc:
            raise WorkflowTemplateError(f"workflow file not found: {path}") from exc
        except json.JSONDecodeError as exc:
            raise WorkflowTemplateError("workflow file is not valid JSON") from exc
        if not isinstance(value, dict):
            raise WorkflowTemplateError("workflow root must be an object")
        return value

    @staticmethod
    def _validate(workflow: dict[str, Any]) -> None:
        for node_id, expected_type in REQUIRED_NODES.items():
            node = workflow.get(node_id)
            if not isinstance(node, dict) or node.get("class_type") != expected_type:
                raise WorkflowTemplateError(
                    f"workflow node {node_id} must be {expected_type}"
                )
            if not isinstance(node.get("inputs"), dict):
                raise WorkflowTemplateError(f"workflow node {node_id} has no inputs")

    def render(
        self,
        request: ParsedImageToVideoRequest,
        *,
        uploaded_image: str,
        video_id: str,
    ) -> dict[str, Any]:
        if VIDEO_ID_PATTERN.fullmatch(video_id) is None:
            raise ValueError("invalid video id")
        if not uploaded_image or uploaded_image.startswith("/") or ".." in uploaded_image.split("/"):
            raise ValueError("invalid uploaded image name")
        workflow = copy.deepcopy(self.template)
        width, height = (int(value) for value in request.size.split("x", 1))
        workflow["269"]["inputs"]["image"] = uploaded_image
        workflow["320:276"]["inputs"]["noise_seed"] = request.seed
        workflow["320:277"]["inputs"]["noise_seed"] = request.second_seed
        workflow["320:299"]["inputs"]["value"] = height
        workflow["320:300"]["inputs"]["value"] = request.fps
        workflow["320:301"]["inputs"]["value"] = request.seconds
        workflow["320:312"]["inputs"]["value"] = width
        workflow["320:319"]["inputs"]["value"] = request.rewritten_prompt
        if request.negative_prompt is not None:
            workflow["320:313"]["inputs"]["text"] = request.negative_prompt
        # External Qwen rewriting is authoritative; disable the workflow's second rewrite.
        workflow["320:328"]["inputs"]["value"] = False
        workflow["75"]["inputs"]["filename_prefix"] = f"video/agent/i2v/{video_id}"
        return workflow
