...
from .rewriter import GPUStackPromptRewriter, PromptRewriter
from .schemas import ImageToVideoOptions, ImageToVideoResult
from .service import ImageToVideoProgress, ImageToVideoService
from .settings import ImageToVideoSettings


class ChatCompletionRequest(OpenAIChatCompletionRequest):
    model: str = MODEL_ID
    video: ImageToVideoOptions = Field(default_factory=ImageToVideoOptions)
...
def create_app(
    settings: ImageToVideoSettings | None = None,
    *,
    client: ComfyUIClient | Any | None = None,
    rewriter: PromptRewriter | Any | None = None,
) -> FastAPI:
    selected = settings or ImageToVideoSettings()
    owns_client = client is None
    selected_client = client or ComfyUIClient(
        base_url=selected.comfyui_i2v_base_url,
        public_base_url=selected.public_base_url,
        request_timeout_seconds=selected.comfyui_i2v_request_timeout_seconds,
    )
    selected_rewriter = rewriter or GPUStackPromptRewriter(selected)
    service = ImageToVideoService(selected, selected_client, selected_rewriter)
    ...
    application = FastAPI(
        title="Direct ComfyUI Image-to-Video Agent",
        version="0.1.0",
        description="LangGraph LTX 2.3 image-to-video worker with bounded controls.",
        lifespan=lifespan,
    )
        ...
        return JSONResponse(
            {
                ...
                "agent": "comfyui-image-to-video",
                "model": MODEL_ID,
                "comfyui": "ready" if ready else "unavailable",
                "prompt_rewrite_model": selected.comfyui_i2v_prompt_rewrite_model,
            },
    ...
    async def create_chat_completion(request: ChatCompletionRequest) -> Any:
        ...
        if is_readiness_probe(request.messages):
            content = (
                "comfyui-image-to-video-agent 已就绪。请上传一张图片并描述运动、"
                "镜头和视频参数；时长最多15秒。"
            )
            if request.stream:
...
async def _stream_generation(
    ...
    try:
        async for event in service.stream(
            ...
            if isinstance(event, ImageToVideoProgress):
                yield _sse_delta(
                    completion_id,
                    created,
                    request.model,
    ...
    except (ValueError, ComfyUIRequestError) as exc:
        yield _sse_delta(completion_id, created, request.model, content=str(exc))
    except Exception:  # noqa: BLE001 - stream must end with a sanitized error
        yield _sse_delta(
            completion_id,
            created,
            request.model,
...
def _video_metadata(result: ImageToVideoResult) -> dict[str, Any]:
    return {
        "id": result.video_id,
        ...
        "seconds": result.request.seconds,
