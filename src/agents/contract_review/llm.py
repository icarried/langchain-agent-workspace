...
MODEL_CONFIGS = {
    "deepseek": ModelConfig(
        ...
        notes="DeepSeek API 使用 OpenAI-compatible 接口；合同按章节分块和维度审查，便于证据定位。",
    ),
...
def create_chat_model(provider: str = "deepseek", model: str | None = None) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        model=model or os.getenv("CONTRACT_REVIEW_MODEL") or config.model,
        api_key=api_key,
        base_url=os.getenv("CONTRACT_REVIEW_BASE_URL") or config.base_url,
        temperature=0,
    )
