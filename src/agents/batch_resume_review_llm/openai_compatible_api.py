...
def stream_chat_completion(
    ...
    for path in review_request.resume_paths:
        yield _sse_delta(completion_id, created, model, None, f"准备审查：{path}\n")
    yield _sse_delta(completion_id, created, model, None, "\n正在执行两阶段审查，请稍候。\n\n")
...
def _extract_resume_paths(text: str) -> list[str]:
    block = _extract_resume_block(text)
...
def _extract_resume_block(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        if _starts_section(line, "简历文件") or _starts_section(line, "简历路径"):
            collecting = True
            line = _strip_section_label(line)
        elif _starts_section(line, "岗位要求") or _starts_section(line, "输出要求"):
            collecting = False
