...
MODEL_CONFIGS = {
    "deepseek": ModelConfig(
        ...
        notes="DeepSeek API 使用 OpenAI-compatible 接口；简历按块审查可保留证据并降低遗漏。",
    ),
...
def create_chat_model(provider: str = "deepseek", model: str | None = None) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        model=model or os.getenv("RESUME_REVIEW_MODEL") or config.model,
        api_key=api_key,
        base_url=os.getenv("RESUME_REVIEW_BASE_URL") or config.base_url,
        temperature=0,
    )
